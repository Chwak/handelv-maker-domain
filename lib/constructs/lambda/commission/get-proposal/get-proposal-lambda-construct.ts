...
export interface GetProposalLambdaConstructProps {
  environment: string;
  regionCode: string;
...
export class GetProposalLambdaConstruct extends Construct {
  public readonly function: NodejsFunction;

  constructor(scope: Construct, id: string, props: GetProposalLambdaConstructProps) {
    super(scope, id);

    const functionName = `${props.environment}-${props.regionCode}-maker-domain-get-proposal-lambda`;
    ...
    this.function = new NodejsFunction(this, 'Function', {
      ...
      handler: 'handler',
      entry: path.join(__dirname, '../../../../functions/lambda/commission/get-proposal/get-proposal-lambda.ts'),
      environment: {
      ...
      description: 'Get a single commission proposal (maker or owning collector)',
    });
