 ...
 * Publishes PENDING outbox events to EventBridge (10-minute Safety Net schedule)
 */
export class RepublishLambdaConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: RepublishLambdaConstructProps) {
    ...
    const role = new iam.Role(this, "RepublishLambdaRole", {
      ...
      description: "IAM role for Republish Lambda",
      inlinePolicies: {
    ...
    this.function = new NodejsFunction(this, "RepublishFunction", {
      ...
      description: "Republish Lambda: Publish PENDING outbox events to EventBridge",
    });
    ...
    this.scheduleRule = new events.Rule(this, "RepublishScheduleRule", {
      ...
      description: `Republish Lambda: Send PENDING events (10-min Safety Net) for ${props.domainName}`,
      schedule,
      enabled: true,
    });
    ...
    this.failedOutboxAlarm = new cloudwatch.Alarm(this, "RepublishFailedAlarm", {
      ...
      alarmDescription: "Outbox event hit retry cap. Manual intervention required.",
    });
